...
typedef enum { eNotWaitingNotification = 0, eWaitingNotification, eNotified } eNotifyValue;
...
typedef struct tskTaskControlBlock {
	...
	BaseType_t xUsingStaticallyAllocatedStack; /* Set to pdTRUE if the stack is a statically allocated array, and
	                                              pdFALSE if the stack is dynamically allocated. */
#endif

	ListItem_t
	    xGenericListItem; /*< The list that the state list item of a task is reference from denotes the state of that
...
static UBaseType_t prvListTaskWithinSingleList(TaskStatus_t *pxTaskStatusArray, List_t *pxList,
...
BaseType_t xTaskGenericCreate(
    TaskFunction_t pxTaskCode, const char *const pcName, const uint16_t usStackDepth, void *const pvParameters,
    UBaseType_t uxPriority, TaskHandle_t *const pxCreatedTask, StackType_t *const puxStackBuffer,
    const MemoryRegion_t
        *const xRegions) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
	BaseType_t   xReturn;
	TCB_t *      pxNewTCB;
...
void vTaskPrioritySet(TaskHandle_t xTask, UBaseType_t uxNewPriority)
{
	...
	taskENTER_CRITICAL();
	{
		...
		if (uxCurrentBasePriority != uxNewPriority) {
			...
			if (listIS_CONTAINED_WITHIN(&(pxReadyTasksLists[uxPriorityUsedOnEntry]), &(pxTCB->xGenericListItem))
			    != pdFALSE) {
...
/* Add the idle task at the lowest priority. */
#if (INCLUDE_xTaskGetIdleTaskHandle == 1)
	{
		...
		xReturn = xTaskCreate(prvIdleTask,
		                      ...
		                      &xIdleTaskHandle); /*lint !e961 MISRA exception, justified as it is not a redundant
...
#else
	{
		...
		xReturn = xTaskCreate(prvIdleTask,
		                      ...
		                      (void *)NULL,
		                      (tskIDLE_PRIORITY | portPRIVILEGE_BIT),
		                      NULL); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to
...
char *pcTaskGetTaskName(TaskHandle_t xTaskToQuery) /*lint !e971 Unqualified char types are allowed for strings and
	...
	vTaskSuspendAll();
	{
		...
		if (uxArraySize >= uxCurrentNumberOfTasks) {
			...
			do {
				uxQueue--;
				uxTask
				    += prvListTaskWithinSingleList(&(pxTaskStatusArray[uxTask]), &(pxReadyTasksLists[uxQueue]), eReady);
			...
			uxTask += prvListTaskWithinSingleList(
...
#if (INCLUDE_vTaskDelete == 1)
			{
				...
				uxTask
				    += prvListTaskWithinSingleList(&(pxTaskStatusArray[uxTask]), &xTasksWaitingTermination, eDeleted);
			}
#endif
		...
		if (*pxTicksToWait == portMAX_DELAY) {
		    ...
		    if ((xNumOfOverflows != pxTimeOut->xOverflowCount)
		        && (xConstTickCount >= pxTimeOut->xTimeOnEntering)) /*lint !e525 Indentation preferred as is to make
		                                                               code within pre-processor directives clearer. */
		{
...
static void prvInitialiseTCBVariables(TCB_t *const pxTCB, const char *const pcName, UBaseType_t uxPriority,
                                      ...
                                      const uint16_t usStackDepth) /*lint !e971 Unqualified char types are allowed for
